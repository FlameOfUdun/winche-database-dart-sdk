...
abstract class _LiveListener<TSnapshot> {
  ...
  _LiveFeed? _feed;
  StreamSubscription<List<WireDocument>?>? _feedSub;
  ...
  /// Stores the latest server set and writes it through to the confirmed cache.
  Future<void> _storeServerDocs(List<WireDocument> docs);
...
final class _QueryListener<T> extends _LiveListener<QuerySnapshot<T>> {
  ...
  @override
  Future<void> _storeServerDocs(List<WireDocument> docs) async {
    _serverDocs = docs;
