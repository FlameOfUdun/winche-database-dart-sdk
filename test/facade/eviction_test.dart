...
WincheDatabase _offlineDb({required int maxCachedDocuments}) =>
    WincheDatabase(WincheApp('eviction'))..debugBindStore(
      ConnectionConfig(
...
void main() {
  test(
    'a document with a pending write is pinned and survives eviction',
    () async {
      ...
      expect(
        await db.cache.confirmed('c/keep'),
        isNotNull,
  ...
  test(
    'an active listener pins its members; an unrelated doc is evicted',
    () async {
      ...
      expect(
        await h.db.cache.confirmed('c/m1'),
        isNotNull,
        reason: 'member pinned',
      );
      expect(
        await h.db.cache.confirmed('c/m2'),
        isNotNull,
        reason: 'member pinned',
      );
      expect(
        await h.db.cache.confirmed('c/loose'),
        isNull,
  ...
  test(
    'server query.get returns the full result even when it exceeds the cap',
    () async {
  ...
  test(
    'server getAll returns all fetched docs even when over the cap',
    () async {
      final h = FacadeHarness(maxCachedDocuments: 1);
      h.handler = (f) {
  ...
  test('byte cap evicts unpinned docs over the limit', () async {
    ...
    for (var i = 0; i < 6; i++) {
      await h.db.cache.putConfirmed(
        WireDocument.fromJson(
          wireDoc('c/d$i', wireFields({'label': 'value-$i'}), collection: 'c'),
        ),
      );
    }
    var live = 0;
  ...
  test(
    'startup enforces the byte cap against already-persisted documents',
    () async {
      ...
      for (var i = 0; i < 6; i++) {
        await h1.db.cache.putConfirmed(
          WireDocument.fromJson(
            wireDoc(
              'c/p$i',
              wireFields({'label': 'value-$i'}),
              collection: 'c',
            ),
          ),
        );
      ...
      await h2.db.cache.putConfirmed(
        WireDocument.fromJson(
