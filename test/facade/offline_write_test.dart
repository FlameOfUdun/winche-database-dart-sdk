...
void main() {
  ...
  WincheDatabase offlineDb(LocalStore store) =>
      WincheDatabase(WincheApp('offline-write'))..debugBindStore(
        ConnectionConfig(
  ...
  test(
    'offline set acks locally and is visible to get (hasPendingWrites)',
    () async {
  ...
  test(
    'offline batch commit enqueues all writes, visible via cache query',
    () async {
      final db = offlineDb(FakeLocalStore());
      await (db.batch()
